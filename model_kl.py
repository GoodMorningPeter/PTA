...
def get_circle_masks(pred_label, part_num):
    ...
    for i in range(pred_label.shape[0]):
        ...
        circle_masks.append(tf.stack(multi_class_masks, axis=2)) # 增加了num_class维度(lja)
    return tf.stack(circle_masks) # 增加了batch维度（lja)
...
def softmax(logits, temperature=1e-5):
    """
    softmax
    input: [*, n_class]
    return: flatten --> [*, n_class] an one-hot vector
    """
    y = tf.nn.softmax(logits / temperature, dim=-1)
...
def get_estimate_mu_sigma(X, mask):
    ...
    n = tf.reduce_sum(mask, [1,2], keep_dims=True) + 1e-10
...
def compute_kl(u1, sigma1, u2, sigma2, dim):
    ...
    # sigma Tensor("truediv_61:0", shape=(5, 4), dtype=float32) Tensor("truediv_63:0", shape=(5, 4), dtype=float32)
    # print("mu", u1, u2)
    ...
    term2 = tf.squeeze(delta_u_transpose @ sigma2_matrix_inv @ delta_u)  # (B,)
    term3 = -dim
    term4 = tf.math.log(sigma2_matrix_det) - tf.math.log(sigma1_matrix_det)
    KL = 0.5 * (term1 + term3 + term2 + term4) 
    # print(KL)
    # KL = tf.clip_by_value(KL, 0, 100)
    # if you want to compute the mean of a batch,then,
    KL_mean = tf.reduce_mean(KL)

    #---- 交叉熵
    # H1 = 0.5 * tf.math.log(2 * numpy.pi * numpy.e * sigma1)
    # H1_mean = tf.reduce_mean(H1)
    # CE12 = KL_mean + H1_mean
    #-----
    
    return KL_mean


def partial_kl_loss(y_pred, plus_mask, minus_mask):
    
    mu1, sigma1 = get_estimate_mu_sigma(y_pred, plus_mask)
    mu2, sigma2 = get_estimate_mu_sigma(y_pred, minus_mask)

    n_plus = tf.reduce_sum(plus_mask)
    n_minus = tf.reduce_sum(minus_mask)

    sigma1 = tf.clip_by_value(sigma1, 1e-5, 1e5)
    sigma2 = tf.clip_by_value(sigma2, 1e-5, 1e5)
    mu1 = tf.clip_by_value(mu1, 1e-5, 1e5)
    mu2 = tf.clip_by_value(mu2, 1e-5, 1e5)

    return 0.5*(compute_kl(mu1, sigma1, mu2, sigma2, dim=4) + compute_kl(mu2, sigma2, mu1, sigma1, dim=4))

def Plus_Minus_KL_Loss(y_true, y_pred, dilation_filter, part_num=10):

    y_pred_label = softmax(y_pred)

    # circle_masks = get_circle_masks(y_pred_label, part_num)
    y_large = (
        tf.nn.dilation2d(
            tf.cast(y_pred_label, tf.float64),
            dilation_filter,
            strides=(1, 1, 1, 1),
            rates=(1, 1, 1, 1),
            padding="SAME",
    ...
    y_small = 2 - tf.nn.dilation2d(
        1 - tf.cast(y_pred_label, tf.float64),
        dilation_filter,
        strides=(1, 1, 1, 1),
        rates=(1, 1, 1, 1),
        padding="SAME",
    )
    plus_mask = y_large - tf.cast(y_pred_label, tf.float64)
    minus_mask = tf.cast(y_pred_label, tf.float64) - y_small
...
def LABELED_Plus_Minus_KL_Loss(X, y_label, y_pred, dilation_filter, part_num=10):
    ...
    plus_mask = y_large - y_pred_label
    minus_mask = y_pred_label - y_small
    # y_pred shape (B, W, H, class_num)
    # plus_mask shape (B, W, H, class_num)
    ...
    y_label_small = 2 - tf.nn.dilation2d(
        1 - tf.cast(y_label, tf.float32),
        dilation_filter,
        strides=(1, 1, 1, 1),
        rates=(1, 1, 1, 1),
        padding="SAME",
    )
    label_plus_mask = y_label_large - tf.cast(y_label, tf.float32)
    label_minus_mask = tf.cast(y_label, tf.float32) - y_label_small
    #---
    ...
    loss = tf.clip_by_value(loss, 0.5, 1.5)
    return loss

def INTER_LABELED_Plus_Minus_KL_Loss(X, y_label, y_pred, dilation_filter, part_num=10):
...
def RAW_Plus_Minus_KL_Loss(X, y_pred, dilation_filter, part_num=10):
...
def loss(
    ...
    if loss_type == "weighted_crossentropy":
        segmentation_loss = losses.pixel_wise_cross_entropy_loss_weighted(
            logits, labels, class_weights=[0.1, 0.2, 0.35, 0.35]
        )
    ...
    # TODO(Jintao, Saining, Jiaan): KDE_Loss KL_Loss
    # student_loss, _, _ = RAW_Student_Circle_Loss(images, logits, dilation_filter)
    # kl_loss = RAW_Plus_Minus_KL_Loss(images, logits, dilation_filter)
    # kl_loss = RAW_Plus_Minus_KL_Loss(labels, logits, dilation_filter)
    # kl_loss = LABELED_Plus_Minus_KL_Loss(images, labels, logits, dilation_filter)
    kl_loss = INTER_LABELED_Plus_Minus_KL_Loss(images, labels, logits, dilation_filter)
    # ac_loss += student_loss
    # + student_loss
    # kl_loss = tf.zeros(shape=())
    total_loss = tf.add(segmentation_loss, weights_norm)  # + ac_loss / 10
    if warm_up_done:
        # pass
    
        # total_loss = kl_loss
        # kl_loss = tf.clip_by_value(kl_loss, 1, 40)
        # kl_loss = tf.math.log(kl_loss)
        # total_loss += loss_k / (kl_loss*100)
        kl_loss = tf.sqrt((kl_loss-1)**2) / 10000
        kl_loss = tf.clip_by_value(kl_loss, total_loss/100, total_loss/10)
        total_loss += kl_loss


        # total_loss = sum(student_loss) / loss_k + total_loss
        # kl_loss = sum(student_loss)
...
def evaluation(logits, labels, images, nlabels, loss_type, warm_up_done):
    ...
    total_loss, kl_loss, nowd_loss, weights_norm = loss(
        images,
        logits,
        labels,
        nlabels=nlabels,
